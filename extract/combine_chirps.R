...
m <- data.table(readRDS('~/mortalityblob/mortnew/chirps/rastToPts.RDS'))
#Convert to CMC
names(m)[3:length(names(m))] <- 12*(as.numeric(substr(names(m)[3:length(names(m))], 2, 5)) - 1900) + as.numeric(substr(names(m)[3:length(names(m))], 7, 8))

m <- melt(m, id.vars=c('x', 'y'), measure.vars=patterns('\\d+'))
